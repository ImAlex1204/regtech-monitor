...
3. **Store** — persist to SQLite, keyed by URL, tagged with its source regulator, so re-running the pipeline never re-processes (or re-pays for) an announcement it's already seen. A daily GitHub Actions run does this and exports `data/announcements.json` for the dashboard.
...
- The same story occasionally appears under two FCA URLs (e.g. `/news-stories/` and `/enforcement-investigations/`); deduplication is by URL, so such a story is stored twice.
