...
4. **Browse & compare** — a Streamlit dashboard opens with overall KPI tiles, then a **by-regulator breakdown**: side-by-side KPI cards, two weekly trend charts (total and high-risk announcements, one line per regulator), and a grouped bar chart of business-area composition per regulator — the actual point of tracking two jurisdictions side by side, not just two filtered lists. Below that, an upcoming-deadlines table and the full announcement list, both grouped by regulator first (soonest-deadline / most-recent second) so the two jurisdictions read as two clearly separated blocks rather than an interleaved feed. The announcement list also carries a truncated summary column, so scanning a whole filtered result set shows *why* each row was classified the way it was without a click per row; clicking one still expands the full untruncated summary and a link to the source. The interface chrome (labels, headers, metrics, chart titles, `risk_level` and `business_area` values) toggles between Chinese and English; `summary` itself doesn't re-translate on toggle — it was written in English to begin with.
